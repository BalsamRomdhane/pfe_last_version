...
def get_ollama_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {'available': True, 'model': OLLAMA_MODEL, 'models': models, 'url': OLLAMA_URL}
        return {'available': False, 'model': OLLAMA_MODEL, 'reason': f'HTTP {resp.status_code}'}
    except Exception as exc:
        return {'available': False, 'model': OLLAMA_MODEL, 'reason': str(exc)}
...
def _build_prompt(question: str, context_rules: List[Dict], context_evidence: List[Dict]) -> str:
    ...
    return (
        "You are a compliance expert assistant. Answer the following question "
        "based on the compliance rules and evidence provided.\n\n"
...
def _call_ollama_generate(
    ...
    """Send a non-streaming request to Ollama and return the answer dict."""
    try:
        resp = requests.post(
...
def stream_compliance_answer(
    ...
    """Stream compliance answer tokens via Ollama."""
    if not get_ollama_status().get('available'):
    ...
    prompt = _build_prompt(question, context_rules, context_evidence)
    yield from _stream_ollama_tokens(question, prompt, context_rules, context_evidence)
...
def _stream_ollama_tokens(
    ...
    """Low-level token streaming from Ollama."""
    try:
        resp = requests.post(
            f'{OLLAMA_URL}/api/generate',
            json={
...
def _keyword_fallback(
    ...
    relevant_ev = [
        ...
        if any(w in e.get('evidence_text', '').lower() for w in q_words)
    ]
    ...
    if relevant_rules or relevant_ev:
        ...
        if relevant_ev:
            parts.append(
                'Related evidence:\n' + '\n'.join(
                    f"- {e.get('rule_title', '')}: {e.get('evidence_text', '')[:150]}"
