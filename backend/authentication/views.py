...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            logger.info(f"Login attempt for: {login_field}")
            ...
            try:
                ...
                if not bypass_keycloak:
                    try:
                        ...
                        logger.info(f"Attempting Keycloak authentication for {login_field}")
                        token_response = keycloak_service.authenticate_user(login_field, password)
                        access_token = token_response.get('access_token')
                        logger.info(f"Keycloak authentication successful for {login_field}")
                        ...
                        logger.warning(f"Keycloak auth failed: {error_msg}, trying Django fallback")
                        try:
                            token_response = keycloak_service.authenticate_user_django_only(login_field, password)
                            access_token = token_response.get('access_token')
                            logger.info(f"Django fallback authentication successful for {login_field}")
                        except Exception as django_error:
                            logger.error(f"Both Keycloak and Django auth failed: {django_error}")
                            return Response({
                                'error': 'Authentication failed',
                                'detail': str(django_error),
                                'keycloak_error': error_msg,
                                'note': 'Keycloak is unavailable. Django fallback also failed. Ensure user exists with correct password.'
                            }, status=status.HTTP_401_UNAUTHORIZED)
                else:
                    # Django-only authentication
                    try:
                        logger.info(f"Attempting Django-only authentication for {login_field}")
                        token_response = keycloak_service.authenticate_user_django_only(login_field, password)
                        access_token = token_response.get('access_token')
                        logger.info(f"Django authentication successful for {login_field}")
                    except Exception as django_error:
                        logger.error(f"Django authentication failed: {django_error}")
                        return Response({
                            'error': 'Authentication failed',
                            'detail': str(django_error)
                        }, status=status.HTTP_401_UNAUTHORIZED)
                ...
                try:
                    ...
                    logger.error(f"Failed to get user profile: {profile_error}")
                ...
                if profile.get('is_first_login'):
                    try:
                        ...
                        logger.error(f"Failed to send first login email: {reset_error}")
                        return Response({
                ...
                logger.info(f"Login successful for {login_field}, returning token and profile")
                return Response({
                ...
                logger.error(f"Unhandled login error: {detail}")
