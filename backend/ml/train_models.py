...
def load_text_dataset(standard=None, norme_id=None):
    texts = []
    y = []
    ...
    for sample in samples:
        if not sample.rule_text and not sample.evidence_text and not sample.document_text:
            continue
        text = " ".join(
            part for part in [sample.rule_text, sample.evidence_text, sample.document_text] if part
        )
        if not text.strip():
            continue
...
def train_all_models(standard=None, norme_id=None):
    ...
    X_text, y_text = load_text_dataset(standard, norme_id)
    text_warning = None
    if len(X_text) >= 20 and len(np.unique(y_text)) >= 2:
        try:
