...
class UserDetailView(APIView):
    ...
    def put(self, request, user_id):
        ...
        serializer = UserUpdateSerializer(data=request.data, context={'user_id': user_id})
        if not serializer.is_valid():
            return Response({
                'error': 'Invalid input',
                'detail': serializer.errors
        ...
        try:
            with transaction.atomic():
                ...
                if profile.keycloak_id and 'username' in serializer.validated_data:
                    try:
                        keycloak_service = KeycloakService()
                        keycloak_service.update_user_in_keycloak(
                            profile.keycloak_id,
                            username=serializer.validated_data['username'],
                            first_name=serializer.validated_data.get('first_name'),
                            last_name=serializer.validated_data.get('last_name')
                ...
                if 'is_first_login' in serializer.validated_data:
                    profile.is_first_login = serializer.validated_data['is_first_login']
